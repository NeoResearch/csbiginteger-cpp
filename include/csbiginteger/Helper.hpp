...
class Helper {
 ...
 public:  // helper
  ...
  // TODO(igormcoelho): test (only indirectly tested now)
  static std::string revertHexString(std::string shex) {
    // if needs padding
    if (shex.length() % 2 == 1)
      shex.insert(0, "0");  // TODO(igormcoelho): beware if that makes sense for
                            // negative... perhaps 'f' is better
    std::string hex = shex;
 ...
 public:  // helper
  // TODO(igormcoelho): from neopt
  static cs_vbyte HexToBytes(const std::string& hex) {
    // TODO(igormcoelho): implement (begin 0x)
