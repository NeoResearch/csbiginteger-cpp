...
#include <csbiginteger/csBigIntegerLibClass.hpp>
...
// empty bytearray initialization is not considered error (but is zero as
// default)
...
// test('constructor csBigInteger(4293967296).toHexString() equals
// "c0bdf0ff00"', () => {
...
// test('test 255: csBigInteger(0.00000255*100000000).toHexString() equals
// "ff00"', () => {
//   expect(new
//   csBigInteger(0.00000255*100000000).toHexString()).toEqual("ff00");
// });

TEST_CASE(
    "csBISerializeTests:  BigInteger0p00000255MUL100000000_toHexStr_Sff00") {
...
// from javascript:
// c0bdf0ffffffffff will not work currently, because precision is not 64-bit
// (around ~60 bit) So, testing will be done (for now), up to 6-bytes!
...
// test('csBigInteger(9007199254740991) equals 9007199254740991 (number is safe
// for JavaScript)', () => {
//   expect(new
//   csBigInteger(9007199254740991).valueOf()).toBe(9007199254740991);
// });
...
// test('csBigInteger(9007199254740992) to throw exception (number is unsafe for
// JavaScript)', () => {
//   expect( function(){ new csBigInteger(9007199254740992).valueOf()
//   }).toThrow(new Error("csBigInteger assertion failed: unsafe number"));
// });
...
// test('csBigInteger(-9007199254740991) equals -9007199254740991 (number is
// safe for JavaScript)', () => {
//   expect(new
//   csBigInteger(-9007199254740991).valueOf()).toBe(-9007199254740991);
// });
...
// test('csBigInteger(-9007199254740992) to throw exception (number is unsafe
// for JavaScript)', () => {
//   expect( function(){ new csBigInteger(-9007199254740992).valueOf()
//   }).toThrow(new Error("csBigInteger assertion failed: unsafe number"));
// });
...
//   expect(new
 //   csBigInteger('17976931348623159077293051907890247336179769789423065727343008115773267580550096313270847\
...
TEST_CASE("csBISerializeTests:  BigInteger_big_toHexStr_length_129_bytes") {
  BigInteger big(
      ...
      "80550096313270847"
      "732240753602112011387987139335765878976881441662249284743063947412437776"
      "789342486548527630221960124609411945308295208"
      "500576883815068234246288147391311054082723716335051068458629823994724593"
      "8479716304835356329624224137215",
      10);
...
TEST_CASE("csBISerializeTests:  BigInteger_big_toHexStr_equals_hex") {
  BigInteger big(
      ...
      "5"
      "80550096313270847"
      "73224075360211201138798713933576587897688144166224928474306394741243777"
      "6"
      "789342486548527630221960124609411945308295208"
      "50057688381506823424628814739131105408272371633505106845862982399472459"
      "3"
      "8479716304835356329624224137215",
      10);
  REQUIRE(big.toHexStr() ==
...
10).asBN()).
toEqual(new
BN('17976931348623159077293051907890247336179769789423065727343008115773267580550096313270847\
732240753602112011387987139335765878976881441662249284743063947412437776789342486548527630221960124609411945308295208\
5005768838150682342462881473913110540827237163350510684586298239947245938479716304835356329624224137215',
10));
});
...
732240753602112011387987139335765878976881441662249284743063947412437776789342486548527630221960124609411945308295208\
5005768838150682342462881473913110540827237163350510684586298239947245938479716304835356329624224137215',
10).asBN().sub(new BN(1))).
toEqual(new
BN('17976931348623159077293051907890247336179769789423065727343008115773267580550096313270847\
732240753602112011387987139335765878976881441662249284743063947412437776789342486548527630221960124609411945308295208\
5005768838150682342462881473913110540827237163350510684586298239947245938479716304835356329624224137214',
10));
});
*/

// test('csBigInteger(big).sub(1).ToHexString equals hexbig', () => {
//   expect(new csBigInteger(new
 //   csBigInteger('17976931348623159077293051907890247336179769789423065727343008115773267580550096313270847\
732240753602112011387987139335765878976881441662249284743063947412437776789342486548527630221960124609411945308295208\
5005768838150682342462881473913110540827237163350510684586298239947245938479716304835356329624224137215',
// 10).asBN().sub(new BN(1))).toHexString()).
...
TEST_CASE("csBISerializeTests:  BigInteger_big_minus_1_equals_hex") {
  ...
  REQUIRE(bigFinal.toHexStr() ==
          "feffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
          "fffffffffffffffffffffffffffffffffffffffffffff"
          "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
          "ffffffffffffffffffffffffffffffffffffffffffffffffffff"
          "fffffffffffffffffffffff00");
...
TEST_CASE("csBISerializeTests: isUnsigned isBigEndian II") {
  cs_vbyte bytes =
      Helper::HexToBytes("cd0815de2ee5e0e1d6d19ed359f589860a9db799");
  // big signed
  BigInteger big_signed(bytes, false, true);
  REQUIRE(big_signed.ToString(10) ==
          "-290978218387399462397957987055587053118878271591");
  // big unsigned
  BigInteger big(bytes, true, true);
  REQUIRE(big.ToString(10) ==
          "1170523418943503455805726845660695966537054271385");
  // TODO: cannot verify the test above! please double check number
  // '1170523418943503455805726845660695966537054271385' and negative
  // counterpart!
}
