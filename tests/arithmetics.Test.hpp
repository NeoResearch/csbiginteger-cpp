...
// from:
// https://docs.microsoft.com/en-us/dotnet/api/system.numerics.biginteger.divide?view=netframework-4.8
...
TEST_CASE("csBIArithmeticsTests:  Online_Pack_Divisor") {
  ...
  BigInteger dividends[] = {
      BigInteger::Multiply(BigInteger(std::numeric_limits<float>::max()), 2),
      BigInteger("90612345123875509091827560007100099", 10), BigInteger::One(),
      BigInteger::Multiply(BigInteger(std::numeric_limits<int>::max()),
                           BigInteger(std::numeric_limits<cs_int64>::max())),
  ...
  REQUIRE(dividends[0].ToString(10) ==
  ...
  REQUIRE((dividends[0] % divisor).ToString(10) ==
  ...
  REQUIRE((dividends[1] % divisor).ToString(10) ==
  ...
  REQUIRE((dividends[3] % divisor).ToString(10) ==
...
// TODO: add these bunch of tests
// https://github.com/dotnet/corefx/tree/master/src/System.Runtime.Numerics/tests/BigInteger
